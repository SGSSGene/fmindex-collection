...
struct Search {
    ...
    void search_next(cursor_t const& cur, int e, BlockIter blockIter, size_t lastRank) const noexcept {
        if (cur.count() == 0) {
            return;
        }
    ...
    void search_next_dir(cursor_t const& cur, int e, BlockIter blockIter, size_t lastRank) const noexcept {

...
// Using 2 bidirectional FM-Indices for search
// optMode:
// - 1: single query search if bidirectional fmindex cursor only has a single result
// - 2: follow single index result if index cursor only has a single result
// - 4: query and index only have a single index
namespace fmc::search_double_index {
...
//        auto qcursors = qcur.extendLeft();
        auto qcursors = [&]() {
            if constexpr (Right) return qcur.extendRight();
            else                 return qcur.extendLeft();
        }();
        auto cursors = [&]() {
            if constexpr (Right) return cur.extendRight();
            else                 return cur.extendLeft();
        }();
...
//        auto qcursorNext = qcur.extendLeft(curQSymb);
        ...
        // match
        if (curQSymb == curISymb && matchAllowed && curQSymb != 0) {
            auto sp = Restore{part, part+1};
        ...
        if (mismatchAllowed) {
            auto se = Restore{e, e+1};
            // search substitutes
            if (curQSymb != curISymb && curQSymb != 0)
            {
                auto s1 = Restore{side[Right].lastRank, curISymb};
                auto s2 = Restore{side[Right].lastQRank, curQSymb};
                auto sp = Restore{part, part+1};
    ...
    void searchPartDirSingleIndex(cursor_t const& cur, qcursor_t const& qcur) const noexcept {
        ...
        if (mismatchAllowed) {
            ...
            auto qcursorNext = [&]() {
                if constexpr (Right) return qcur.extendRight(curISymb);
                else                 return qcur.extendLeft(curISymb);
            }();
            auto sp = Restore{part, part+1};
            auto s1 = Restore{side[Right].lastRank, curISymb};
            auto s2 = Restore{side[Right].lastQRank, curISymb};
...
template <bool Edit, typename Index, typename QIndex, typename search_scheme_t, typename delegate_t>
void search(Index const& index, QIndex const& queryIndex, search_scheme_t const& search_scheme, size_t threshold, size_t optMode, delegate_t&& delegate) {
    for (auto const& search : search_scheme) {
        using search_t = std::decay_t<decltype(search)>;

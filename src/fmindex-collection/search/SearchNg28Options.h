...
/**
 * like search_ng28 but with options relevant for my thesis evaluation
 */
namespace fmc::search_ng28_options {

/**
 * Options:
 * 0x00: no options
 * 0x01: insertion/Allowed condition
 * 0x02: match condition
 * 0x04: report condition
 */
struct Options {
    bool indelCond{};
    bool matchCond{};
...
struct Search {
    ...
    Search(index_t const& _index, query_t const& _query, search_t const& _search, std::vector<size_t> const& _partition, Options _options, delegate_t const& _delegate)
        : index     {_index}
        , query     {_query}
        , search    {_search}
    ...
    auto computeErrorCases() const {
        ...
        if (options.indelCond) {
            ...
            bool const substitutionAllowed = ((Edit && (partitionPart > 1 || lb <= e+1)) || (!Edit && lb <= e+partitionPart)) && mismatchAllowed;
            bool const insertionAllowed    = Edit && substitutionAllowed;
            bool const deletionAllowed     = Edit && mismatchAllowed;
            return std::make_tuple(mismatchAllowed, substitutionAllowed, insertionAllowed, deletionAllowed);
        }
    }
    auto computeMatchCase(size_t nextSymb) const {
        auto const tinfo        = side->info;
        if (options.matchCond) {
    ...
    auto computeReportCond() const {
        auto il = sideImpl[0].info;
        auto ir = sideImpl[2].info;
        if (!Edit || ((il == 'M' or il == 'I') and (ir == 'M' or ir == 'I'))) {
            if (options.reportCond) {
                auto maxError = (e == search.u.back());
                return maxError;
            } else {
                auto insideRange = search.l.back() <= e and e <= search.u.back();
                return insideRange;
            }
        }
        return false;
...
void search_impl(index_t const& index, query_t const& query, search_scheme::Scheme const& search_scheme, std::vector<size_t> const& partition, Options options, delegate_t&& delegate) {
    using cursor_t = select_cursor_t<index_t>;
    ...
    for (auto const& search : search_scheme) {
        bool f = Search<Edit, index_t, query_t, decltype(search), decltype(internal_delegate)>{index, query, search, partition, options, internal_delegate}.run();
        if (f) {
            return;
        }
    }
...
void search_n_impl(index_t const& index, queries_t&& queries, selectSearchScheme_t&& selectSearchScheme, std::vector<size_t> const& partition, Options options, delegate_t&& delegate, size_t n) {
    if (queries.empty()) return;
    if (n == 0) return;
    for (size_t qidx{}; qidx < queries.size(); ++qidx) {
        size_t ct{};
        auto const& search_scheme = selectSearchScheme(queries[qidx].size());
        search_impl<Edit>(index, queries[qidx], search_scheme, partition, options, [&] (auto cur, size_t e) {
            if (cur.count() + ct > n) {
                cur.len = n-ct;
            }
            ct += cur.count();
...
void search(index_t const& index, queries_t&& queries, search_scheme::Scheme const& search_scheme, std::vector<size_t> const& partition, Options options, delegate_t&& delegate, size_t n = std::numeric_limits<size_t>::max()) {
    // function that selects a search scheme
    auto selectSearchScheme = [&]([[maybe_unused]] size_t length) -> auto& {
        return search_scheme;
    };
    search_n_impl<Edit>(index, queries, selectSearchScheme, partition, options, delegate, n);
}

...
namespace occtable {
namespace interleavedEPRV7b_impl {

template <uint64_t _TSigma>
struct Bitvector {

    // number of full length bitvectors needed `2^bitct ≥ TSigma`
    static constexpr auto bitct = required_bits(_TSigma-1);
    // next full power of 2
    static constexpr auto TSigma  = pow(2, bitct);
...
struct OccTable {
    ...
    auto all_ranks(uint64_t idx) const -> std::tuple<std::array<uint64_t, Sigma>, std::array<uint64_t, Sigma>> {
        ...
        auto _rs = std::array<uint64_t, Sigma>{};
        auto _prs = std::array<uint64_t, Sigma>{};
...
namespace interleavedEPRV7b {
template <uint64_t TSigma>
struct OccTable : interleavedEPRV7b_impl::OccTable<TSigma> {
    using interleavedEPRV7b_impl::OccTable<TSigma>::OccTable;

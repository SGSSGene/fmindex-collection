...
struct OccTable {
    ...
    OccTable(std::span<uint8_t const> in_bwt) {
        ...
        static int i {0};
        ++i;
        writeFile("tmp.sdsl.tmp" + std::to_string(i), _bwt);
        sdsl::construct(index, "tmp.sdsl.tmp" + std::to_string(i), 1);

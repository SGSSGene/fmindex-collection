...
//!TODO divsufsort leaks ALPHABET_SIZE definition, destroying the code below
#ifdef ALPHABET_SIZE
#undef ALPHABET_SIZE
#endif

static constexpr auto ALPHABET_SIZE = 1 << CHAR_BIT;
static constexpr auto UNBWT_FASTBITS = 17;

//!TODO LIBSAIS_BUCKETS_INDEX2 can't be a constexpr lambda, because this clashes with openmp
//static constexpr auto LIBSAIS_BUCKETS_INDEX2 = [](auto _c, auto _s) { return ((_c) << 1) + (_s); };
#define LIBSAIS_BUCKETS_INDEX2(_c, _s) ((_c) << 1) + (_s)

static constexpr auto BUCKETS_INDEX4 = [](auto _c, auto _s) { return ((_c) << 2) + (_s); };

static constexpr auto LIBSAIS_PER_THREAD_CACHE_SIZE = 24576;

template <typename sa_sint_t>
struct LIBSAIS_THREAD_CACHE {
        sa_sint_t                       symbol;
        sa_sint_t                       index;
};

template <typename _fast_sint_t, typename _sa_sint_t>
union LIBSAIS_THREAD_STATE {
    using fast_sint_t = _fast_sint_t;
    using sa_sint_t   = _sa_sint_t;
    struct {
        ...
        sa_sint_t *                      buckets;
        LIBSAIS_THREAD_CACHE<sa_sint_t>* cache;
    } state;
...
template <typename _fast_sint_t, typename _sa_sint_t>
struct LIBSAIS_CONTEXT {
    using fast_sint_t  = _fast_sint_t;
    using sa_sint_t    = _sa_sint_t;
    using THREAD_STATE = LIBSAIS_THREAD_STATE<fast_sint_t, sa_sint_t>;
...
template <typename fast_sint_t, typename sa_uint_t>
...
inline T* alloc_aligned(size_t alignment)
{
    auto size = sizeof(T)*number;
    void * address = malloc(size + sizeof(short) + alignment - 1);
    if (address != NULL)
    {
        void * aligned_address = align_up((void *)((ptrdiff_t)address + (ptrdiff_t)(sizeof(short))), alignment);
...
template <typename LIBSAIS_THREAD_STATE>
inline LIBSAIS_THREAD_STATE * alloc_thread_state(typename LIBSAIS_THREAD_STATE::sa_sint_t threads)
{
    using sa_sint_t = typename LIBSAIS_THREAD_STATE::sa_sint_t;

    LIBSAIS_THREAD_STATE *  RESTRICT thread_state    = (LIBSAIS_THREAD_STATE *)alloc_aligned((size_t)threads * sizeof(LIBSAIS_THREAD_STATE), 4096);
    sa_sint_t *             RESTRICT thread_buckets  = (sa_sint_t *)alloc_aligned((size_t)threads * 4 * ALPHABET_SIZE * sizeof(sa_sint_t), 4096);
    auto*                   RESTRICT thread_cache    = (LIBSAIS_THREAD_CACHE<sa_sint_t>*)alloc_aligned((size_t)threads * LIBSAIS_PER_THREAD_CACHE_SIZE * sizeof(LIBSAIS_THREAD_CACHE<sa_sint_t>), 4096);

    if (thread_state != NULL && thread_buckets != NULL && thread_cache != NULL)
    {
        for (sa_sint_t t = 0; t < threads; ++t)
        {
...
template <typename LIBSAIS_THREAD_STATE>
inline void free_thread_state(LIBSAIS_THREAD_STATE * thread_state)
{
...
template <typename LIBSAIS_CONTEXT>
inline LIBSAIS_CONTEXT * libsais_create_ctx_main(typename LIBSAIS_CONTEXT::sa_sint_t threads)
{
    using sa_sint_t    = typename LIBSAIS_CONTEXT::sa_sint_t;
    using THREAD_STATE = typename LIBSAIS_CONTEXT::THREAD_STATE;

    auto* RESTRICT ctx            = alloc_aligned<LIBSAIS_CONTEXT>(64);
    auto* RESTRICT buckets        = alloc_aligned<sa_sint_t, 8 * ALPHABET_SIZE>(4096);
    auto* RESTRICT thread_state   = threads > 1 ? alloc_thread_state<THREAD_STATE>(threads) : NULL;
...
//
//    fast_sint_t i, j; sa_sint_t sum = 0;
//    for (i = LIBSAIS_BUCKETS_INDEX2(0, 0), j = 0; i <= BUCKETS_INDEX2((fast_sint_t)k - 1, 0); i += BUCKETS_INDEX2(1, 0), j += 1)
//    { 
//        bucket_start[j] = sum;
//        sum += buckets[i + LIBSAIS_BUCKETS_INDEX2(0, 0)] + buckets[i + BUCKETS_INDEX2(0, 1)];
//        bucket_end[j] = sum;
//    }
//}
//
...
//    for (i = LIBSAIS_BUCKETS_INDEX2(0, 0); i <= BUCKETS_INDEX2((fast_sint_t)k - 1, 0); i += BUCKETS_INDEX2(1, 0))
//    { 
//        sum0 += buckets[i + LIBSAIS_BUCKETS_INDEX2(0, 0)] + buckets[i + BUCKETS_INDEX2(0, 1)]; buckets[i + BUCKETS_INDEX2(0, 0)] = sum0;
//    }
//}
//
...
//
//        fast_sint_t i, j; sa_sint_t sum = 0;
//        for (i = BUCKETS_INDEX4(0, 0), j = LIBSAIS_BUCKETS_INDEX2(0, 0); i <= BUCKETS_INDEX4(ALPHABET_SIZE - 1, 0); i += BUCKETS_INDEX4(1, 0), j += BUCKETS_INDEX2(1, 0))
//        { 
...
//    for (i = LIBSAIS_BUCKETS_INDEX2(0, 0); i <= BUCKETS_INDEX2((fast_sint_t)k - 1, 0); i += BUCKETS_INDEX2(1, 0))
//    { 
//        sum0 += buckets[i + LIBSAIS_BUCKETS_INDEX2(0, 0)] + buckets[i + BUCKETS_INDEX2(0, 1)];
...
//        sum1 += buckets[i + LIBSAIS_BUCKETS_INDEX2(0, 0)] + buckets[i + BUCKETS_INDEX2(0, 1)];
...
//
//                fast_sint_t i, j;
//                for (i = LIBSAIS_BUCKETS_INDEX2(0, 0), j = BUCKETS_INDEX4(0, 1); i <= BUCKETS_INDEX2(ALPHABET_SIZE - 1, 0); i += BUCKETS_INDEX2(1, 0), j += BUCKETS_INDEX4(1, 0))
//                {
...
//    for (first_lms_suffix = T[first_lms_suffix], i = BUCKETS_INDEX4(0, 0), j = LIBSAIS_BUCKETS_INDEX2(0, 0); i <= BUCKETS_INDEX4((fast_sint_t)first_lms_suffix - 1, 0); i += BUCKETS_INDEX4(1, 0), j += BUCKETS_INDEX2(1, 0))
//    {
...
//#endif
//    for (c = LIBSAIS_BUCKETS_INDEX2(ALPHABET_SIZE - 1, 0); c >= BUCKETS_INDEX2(1, 0); c -= BUCKETS_INDEX2(1, 0))
//    {
...
//    {
//        fast_sint_t l = (fast_sint_t)buckets[LIBSAIS_BUCKETS_INDEX2(c, 1) + BUCKETS_INDEX2(1, 0)] - (fast_sint_t)buckets[BUCKETS_INDEX2(c, 1)];
//        if (l > 0)
//        {
...
//    {
//        fast_sint_t c;
//        for (c = LIBSAIS_BUCKETS_INDEX2((fast_sint_t)k - 2, 0); c >= BUCKETS_INDEX2(0, 0); c -= BUCKETS_INDEX2(1, 0))
//        {
//            fast_sint_t l = (fast_sint_t)buckets[c + LIBSAIS_BUCKETS_INDEX2(1, 1)] - (fast_sint_t)buckets[c + BUCKETS_INDEX2(0, 1)];
//            if (l > 0)
//            {
...
//}
//
//inline int32_t libsais_unbwt_omp(const uint8_t * T, uint8_t * U, int32_t * A, int32_t n, const int32_t * freq, int32_t i, int32_t threads)
//{
//    return libsais_unbwt_aux_omp(T, U, A, n, freq, n, &i, threads);
//}
//
//inline int32_t libsais_unbwt_aux_omp(const uint8_t * T, uint8_t * U, int32_t * A, int32_t n, const int32_t * freq, int32_t r, const int32_t * I, int32_t threads)
//{
...
//    return detail::libsais_unbwt_omp(T, U, A, n, freq, i, threads);
//}
//
///**

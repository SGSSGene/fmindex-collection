...
TEST_CASE("benchmark bitdensity against time", "[sparse-bitvector][!benchmark][bitdensity_vs_time]") {
    ...
    for (auto density : densitySteps) {
        ...
        call_with_templates<AllTypes>([&]<typename Vector>() {
            ...
            results[vector_name].push_back(v / (numberOfIterations / 1'000'000'000.));
        });
    }

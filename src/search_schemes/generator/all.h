...
inline auto all = std::map<std::string, std::function<Scheme(int, int, int, int)>>{
    ...
    { "kianfar",        [](int minError, int maxError, int sigma, int dbSize) { return oss::generator::kianfar(maxError); }},
    { "kucherov-k1",    [](int minError, int maxError, int sigma, int dbSize) { return oss::generator::kucherov(maxError+1, maxError); }},
    { "kucherov-k2",    [](int minError, int maxError, int sigma, int dbSize) { return oss::generator::kucherov(maxError+2, maxError); }},

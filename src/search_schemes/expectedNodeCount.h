 ...
 */
inline long double expectedNodeCount(Search s, size_t sigma, size_t N) {
...
inline long double expectedNodeCount(Scheme const& ss, size_t sigma, size_t N) {
    return std::accumulate(begin(ss), end(ss), static_cast<long double>(0.), [&](long double v, auto const& s) {
        return v + expectedNodeCount(s, sigma, N);
    });
}

inline long double expectedNodeCountEdit(Search s, size_t sigma, size_t N) {
    auto n_max = s.pi.size();
    auto e     = *std::max_element(begin(s.u), end(s.u));
...
inline long double expectedNodeCountEdit(Scheme const& ss, size_t sigma, size_t N) {
    return std::accumulate(begin(ss), end(ss), static_cast<long double>(0.), [&](long double v, auto const& s) {
        return v + expectedNodeCountEdit(s, sigma, N);
    });
}
